...
-- Downloads all UI images from GitHub into the executor workspace on first
-- run. Every run after that loads instantly from local files.
...
for key, file in pairs(ASSET_FILES) do
    ...
    if not isfile(path) then
        missing[#missing + 1] = {key = key, file = file, path = path}
    end
end

if #missing > 0 then
    -- Show a small loading label so the user knows something is happening
    local loadGui = Instance.new("ScreenGui")
    loadGui.Name = "CustomUI_Loading"
    loadGui.ResetOnSpawn = false
    loadGui.DisplayOrder = 9999
    loadGui.Parent = LocalPlayer.PlayerGui

    local lbl = Instance.new("TextLabel")
    lbl.Size = UDim2.new(0, 260, 0, 32)
    lbl.Position = UDim2.new(0.5, -130, 1, -52)
    lbl.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
    lbl.TextColor3 = Color3.fromRGB(0, 210, 255)
    lbl.Font = Enum.Font.GothamBold
    lbl.TextSize = 13
    lbl.Text = "CustomUI — downloading assets…"
    ...
    for i, entry in ipairs(missing) do
        ...
        local ok, res = pcall(function()
            return game:HttpGet(BASE_URL .. entry.file, true)
        end)
        if ok and res and #res > 0 then
            writefile(entry.path, res)
        end
    end
...
-- Build the asset table using getcustomasset
local A = {}
for key, file in pairs(ASSET_FILES) do
...
-- ── Slider ────────────────────────────────────────────────────────────────
-- TDS layout: title row on top, then below: [value box LEFT] [track RIGHT]
function CM:AddSlider(keyOrOpts, opts2)
    ...
    -- bottom row: value box (left) + track (right)
    local rowY = desc and 38 or 26
    local BOX_W = 46  -- width of the value box

    -- value box — dark square on the LEFT, TDS style
    ...
    -- track — sits to the right of the value box, vertically centered in 28px
    local trackX = BOX_W + 8
    local track = New("Frame", {
        BackgroundColor3 = Color3.fromRGB(45, 45, 45),
        Size     = UDim2.new(1, -trackX, 0, 6),
        Position = UDim2.new(0, trackX, 0, rowY + 11),  -- 11 = (28-6)/2
        BorderSizePixel = 0,
    }, r)
    Corner(track, 3)
    ...
    -- filled portion of track (no fill tint — TDS track is plain gray)
    ...
    -- thumb — medium gray rounded rect, taller than track (TDS style)
    local THUMB_W, THUMB_H = 14, 24
    local thumb = New("Frame", {
        BackgroundColor3 = Color3.fromRGB(130, 130, 130),
        Size     = UDim2.new(0, THUMB_W, 0, THUMB_H),
        Position = UDim2.new(p0, -THUMB_W/2, 0.5, -THUMB_H/2),
        BorderSizePixel = 0,
        ZIndex   = 5,
    }, track)
    Corner(thumb, 4)
    ...
    local function setVal(v, fire)
        ...
        thumb.Position  = UDim2.new(p, -THUMB_W/2, 0.5, -THUMB_H/2)
