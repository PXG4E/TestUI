...
function Library:CreateWindow(opts)
    ...
    local function selectTab(i)
        ...
        for j,btn in ipairs(btns) do
            ...
            if on then
                Grad(btn,T.TabActive,T.AccentDark)
                Tween(btn:FindFirstChild("__lbl"),{TextColor3=Color3.new(1,1,1)},0.15)
            else
                Grad(btn,T.TabInactive,Color3.fromRGB(20,20,20))
                Tween(btn:FindFirstChild("__lbl"),{TextColor3=T.TxtSub},0.15)
            end
        end
    end
    ...
    function Win:AddTab(name)
        ...
        -- TDS sidebar tab button: icon + text, full width
        local btn=New("TextButton",{
            Text="", AutoButtonColor=false,
            BackgroundColor3=Color3.new(1,1,1),
        ...
        table.insert(pages,page); table.insert(btns,btn)

        btn.MouseButton1Click:Connect(function() selectTab(idx) end)
        btn.MouseEnter:Connect(function()
            if active~=idx then Grad(btn,Color3.fromRGB(34,34,34),Color3.fromRGB(24,24,24)) end
        end)
        btn.MouseLeave:Connect(function()
            if active~=idx then Grad(btn,T.TabInactive,Color3.fromRGB(20,20,20)) end
        end)
